...
def _telegram_ready():
    ...
    if not config.TELEGRAM_BOT_TOKEN or not config.TELEGRAM_CHAT_ID:
        # 텔레그램 설정이 없어도 프로그램 전체가 멈추면 안 되므로 전송만 건너뛴다.
...
def send_message(text):
    ...
    if not _telegram_ready():
        # 텔레그램 없이 테스트할 때도 어떤 메시지가 갈 예정이었는지 콘솔에 보여준다.
        print(f"[콘솔 메시지] {text}")
        return False
    ...
    # Telegram Bot API의 sendMessage 엔드포인트로 POST 요청을 보낸다.
...
def send_photo(photo_path, caption=""):
    ...
    # Telegram Bot API의 sendPhoto 엔드포인트로 이미지 파일을 multipart 전송한다.
