...
# 예: captures/, logs/, models/ 같은 상대 경로는 모두 이 폴더 기준으로 계산된다.
BASE_DIR = Path(__file__).resolve().parent

# ---------------------------------------------------------------------------
# GPIO 핀 설정
# gpiozero는 BCM GPIO 번호를 사용한다.
# 라즈베리파이 물리 핀 번호가 아니라 BCM 번호이다.
# PIR Signal은 GPIO16, 부저 제어선은 GPIO18로 연결한다.
# ---------------------------------------------------------------------------
PIR_SENSOR_PIN = 16
...
# 모델 파일은 GitHub에 직접 올리지 않고 models/ 폴더에 따로 준비한다.
# 준비 방법: 수업자료 OpencvDnn 저장소에서 복사하거나 python3 download_models.py 실행.
# ---------------------------------------------------------------------------
...
# USB 웹캠 설정이다. Raspberry Pi에서 보통 0번으로 열린다.
# 웹캠이 여러 개이거나 0번으로 열리지 않으면 AIOT_CAMERA_INDEX 환경변수로 바꾼다.
...
# 실행 예: export AIOT_SHOW_CAMERA_WINDOW=1
...
# 실행 전에 export TELEGRAM_BOT_TOKEN="..." / export TELEGRAM_CHAT_ID="..." 형태로 설정한다.
# ---------------------------------------------------------------------------
...
# True이면 라즈베리파이 GPIO 없이도 print 출력으로 프로그램 흐름을 확인할 수 있다.
# ---------------------------------------------------------------------------
