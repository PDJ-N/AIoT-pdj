...
if not config.MOCK_MODE:
    # 실제 라즈베리파이에서만 GPIO 제어 객체를 만든다.
    # MOCK_MODE=True이면 부저 대신 콘솔 출력으로 동작을 확인한다.
    try:
...
def buzzer_on():
    ...
    if config.MOCK_MODE:
        # mock 상태에서는 실제 GPIO 출력 없이 한 번만 ON 메시지를 출력한다.
...
def buzzer_off():
    ...
    global _mock_buzzer_is_on
    if config.MOCK_MODE:
        # mock 상태에서는 실제 GPIO 출력 없이 한 번만 OFF 메시지를 출력한다.
...
def beep(duration):
    ...
    try:
        # try/finally를 사용하여 중간에 오류가 나도 부저가 꺼지도록 한다.
        buzzer_on()
        time.sleep(duration)
    finally:
        buzzer_off()
