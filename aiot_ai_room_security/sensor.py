"""PIR 인체감지 센서를 읽는 모듈."""

import config


if not config.MOCK_MODE:
    # 실제 라즈베리파이에서 실행할 때만 gpiozero를 불러온다.
    # MOCK_MODE=True이면 PC에서도 import 오류 없이 코드 흐름을 테스트할 수 있다.
    try:
        from gpiozero import MotionSensor
    except ImportError as exc:
        raise RuntimeError(
            "gpiozero를 불러올 수 없습니다. "
            "라즈베리파이에서 'pip install gpiozero'를 실행하거나 "
            "config.py의 MOCK_MODE를 True로 바꾸세요."
        ) from exc

    # PIR 센서의 Signal 핀이 GPIO16에 연결되어 있어야 한다.
    _pir_sensor = MotionSensor(config.PIR_SENSOR_PIN)
else:
    _pir_sensor = None


def read_motion():
    """PIR 센서 값이 1이면 True, 움직임이 없으면 False를 반환한다."""
    if config.MOCK_MODE:
        # 콘솔 입력 모드에서는 사용자가 y/n으로 움직임 감지를 흉내 낼 수 있다.
        if config.MOCK_CONSOLE_INPUT:
            answer = input("[MOCK PIR] 움직임 감지로 처리할까요? (y/n): ").strip().lower()
            return answer in {"y", "yes", "1", "true"}

        # 자동 mock 모드에서는 config.MOCK_MOTION 값으로 센서 상태를 대신한다.
        print(f"[MOCK PIR] 움직임 감지값: {int(config.MOCK_MOTION)}")
        return config.MOCK_MOTION

    # gpiozero MotionSensor의 value는 움직임 감지 시 1, 감지 없음이면 0이다.
    return _pir_sensor.value == 1
